                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    @if(empty($playlists))
                        <div class="text-center py-8">
                            ...
                            <a href="{{ route('auth.youtube') }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 active:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150">
                    ...
                    @else
                        <h3 class="text-lg font-medium mb-6">{{ __("Your YouTube Playlists") }}</h3>
                            ...
                            @foreach($playlists as $playlist)
                                <div class="bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300">
                                    @if(isset($playlist['thumbnail']))
                                        <img src="{{ $playlist['thumbnail'] }}" alt="{{ $playlist['title'] }}" class="w-full h-40 object-cover">
                                    @else
                                    ...
                                    @endif
                                    <div class="p-4">
                                        <h4 class="font-semibold text-lg mb-2 truncate">{{ $playlist['title'] }}</h4>
                                        <p class="text-sm text-gray-600 dark:text-gray-300 mb-3 line-clamp-2">{{ $playlist['description'] }}</p>
                                        <div class="flex justify-between items-center">
                                            <span class="text-xs bg-blue-100 dark:bg-blue-800 text-blue-800 dark:text-blue-100 px-2 py-1 rounded-full">
